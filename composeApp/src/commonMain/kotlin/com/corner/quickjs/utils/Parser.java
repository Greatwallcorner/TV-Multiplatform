...
import com.corner.quickjs.bean.Cache;
import com.corner.quickjs.bean.Info;
...
public class Parser {
    ...
    public String parseDomForUrl(String html, String rule, String addUrl) {
        ...
        if (rule.contains("&&")) {
            ...
            rule = StringUtils.join(excludes, "&&");
        }
        ...
        if (StringUtils.isEmpty(option)) return elements.outerHtml();
                ...
                if (!result.isEmpty() && !addUrl.isEmpty()) {
                    if (JOIN_URL.matcher(s).find() && !SPEC_URL.matcher(result).find()) {
                        if (result.contains("http")) {
                            ...
                            result = Urls.INSTANCE.convert(addUrl, result);
                        }
                    }
                }
                if (!result.isEmpty()) {
    ...
    private Elements parseOneRule(Document doc, String parse, Elements elements) {
        ...
        if (elements.isEmpty()) {
            elements = doc.select(info.getRule());
        } else {
            elements = elements.select(info.getRule());
        }
        if (parse.contains(":eq")) {
            if (info.getIndex() < 0) {
                elements = elements.eq(elements.size() + info.getIndex());
            } else {
                elements = elements.eq(info.getIndex());
            }
        }
        if (info.getExcludes() != null && !elements.isEmpty()) {
            elements = elements.clone();
            for (int i = 0; i < info.getExcludes().size(); i++) {
                elements.select(info.getExcludes().get(i)).remove();
            }
        }
        return elements;
    }
