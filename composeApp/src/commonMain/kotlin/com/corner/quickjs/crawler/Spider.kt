...
class Spider(private val key: String, private val api: String, private val loader: URLClassLoader?) :
    ...
    override fun categoryContent(tid: String, pg: String, filter: Boolean, extend: HashMap<String, String>): String {
        try {
            val obj = submit<JSObject> { JSUtil.toObj(ctx, extend) }.get()
    ...
    private fun createCtx() {
        ...
        ctx!!.apply {
            setConsole(Console())
            evaluate(Asset.read("js/lib/http.js"))
    ...
    private fun invokeMultiple(clz: Class<*>, jsObj: JSObject) {
        for (subClz in clz.declaredClasses) {
            ...
            val subObj = ctx!!.createNewJSObject()
    ...
    private fun createObj() {
        ...
        jsObject = ctx!!.getProperty(ctx!!.globalObject, spider) as JSObject
    }
    ...
    private fun cfg(ext: String?): JSObject {
        ...
        else cfg.setProperty("ext", ctx!!.parse(ext) as JSObject)
        return cfg
    }

    private fun proxy1(params: Map<String?, String?>?): Array<Any> {
        val `object` = JSUtil.toObj(ctx, params)
        val array = JSONArray((jsObject!!.getJSFunction("proxy").call(`object`) as JSArray).stringify())
        val headers = if (array.length() > 3) Json.toMap(array.optString(3)) else null
        val base64 = array.length() > 4 && array.optInt(4) == 1
        val result = arrayOf<Any>(array.optInt(0), array.optString(1), getStream(array.opt(2), base64), headers ?: mapOf<String,String>())
        return result
    }
